...
% Add extra parameters
    ...
    if isfield(CBPdata, 'experimentname')
        defaultexpname = CBPdata.experimentname;
    else
...
% Validate number of channels <= 4
    ...
    while size(CBPdata.rawdata.data, 1) > 4
...
% Don't have them change it, just give them a note
    ...
    if params.clustering.num_waveforms > length(CBPInternals.cells_to_plot)
        % MATLAB goofy multiline string stuff
        message = {sprintf('NOTE: These parameters call for %d starting clusters. The default setting in this program is to plot the first %d clusters in the calibration window.\n\n', params.clustering.num_waveforms, length(CBPInternals.cells_to_plot)), ...
                   sprintf('* You can change the cells being plotted by clicking on the "Cell Plot Info..." button in the calibration window.\n'), ...
                   sprintf('* You can also change the number of starting clusters by changing params.clustering.num_waveforms.\n\n'), ...
                   sprintf('Note that every cell is still being calculated internally. This setting only represents which cells are being plotted graphically for calibration and diagnostic purposes. Note that you can change the cells being plotted midway through the program to compare different ones.\n')};
