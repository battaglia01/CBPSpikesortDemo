function TimingComparisonPlotOld(command)
...
% Set up basics
    ...
    spike_amps = CBPdata.waveform_refinement.spike_amps_thresholded;
    spike_traces = CBPdata.waveform_refinement.spike_traces_thresholded;
    ...
    %%@ note - for now, hard-coded to just see the first thing mapped to
    %%@ each ground truth category
    %%@ this happens in a few places - search for more %%@ comments
    get_best_ordering_cl = @(x) min(find(best_ordering_cl(:, x)));
    get_best_ordering_cbp = @(x) min(find(best_ordering_cbp(:, x)));
...
% Plot Timeseries data (top-left subplot)
    ...
    for n=1:num_plot_cells
        ...
        %%@ could maybe condense this
        if c > size(best_ordering_cbp, 2);
            cbp_c = Inf;
        else
            cbp_c = get_best_ordering_cbp(c);
        end
...
%         cbp_c = get_best_ordering_cbp(c);
        ...
        % Clustering indicators
                ...
                % CBP indicators
        if c > size(best_ordering_cl, 2);
            cl_c = Inf;
        else
            cl_c = get_best_ordering_cl(c);
        end
