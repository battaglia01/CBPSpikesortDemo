function [spike_time_array, spike_mags] = ...
    ConvertSpikeTimesFromCell(spike_time_array_cell, spike_mags_cell, ...
...
% Arguments
% spike_time_array_cell: num_snippet x 1 cell array. Each element is a
% num_features x 1 cell array with spike times relative to a snippet center
%
% spike_mags_cell : cell array with same structure as spike_time_array_cell
...
if (length(spike_time_array_cell) ~= length(snippet_centers))
...
num_snippets = length(spike_time_array_cell);
num_features = length(spike_time_array_cell{1});
...
for snippet_num = 1 : num_snippets
    for feature_num = 1 : num_features
            spikes = spike_time_array_cell{snippet_num}{feature_num};
...
% Allocate the memory
spike_time_array = cell(num_features, 1);
spike_mags = cell(num_features, 1);
for feature_num = 1 : num_features
    spike_time_array{feature_num} = zeros(spike_counts(feature_num), 1);
