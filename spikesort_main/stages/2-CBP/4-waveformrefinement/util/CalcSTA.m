...
for b = 1:nblocks
    ...
    xi = bsxfun(@plus, sts(:), xsta); % minkowski sum of both sets
    snippets = interp1(x, data, xi, opts.interpmethod, opts.extrapval);
    if length(sts(:)) > 1
        snippets = sum(snippets,1);
    end
    sta = sta + reshape(snippets, lsta, nchan);
end

%%@ Added to avoid NaN's if no spikes detected
if nspikes ~= 0
    sta = sta / nspikes;
end