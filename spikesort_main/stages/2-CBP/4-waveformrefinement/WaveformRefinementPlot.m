...
% Set up basics
    % set up local vars
    true_num_cells = CBPdata.waveformrefinement.num_waveforms;
    ...
    cluster_matching_perm = CBPdata.waveformrefinement.cluster_matching_perm;
...
%%@ We have two different methods to make this comparison
    t1 = CreateCalibrationTab('Waveform Refinement, Comparison', 'WaveformRefinement');

    % first, get *all* centroids, not just those being plotted
    plot_centroids = [];
    all_centroids = [];
    for n=1:length(CBPdata.waveformrefinement.final_waveforms)
        all_centroids(:, end+1) = ...
...
%% -------------------------------------------------------------------------
% Plot Refined Waveforms (Tab 2)
    t2 = CreateCalibrationTab('Waveform Refinement, Waveforms', 'WaveformRefinement');
    ...
    for n = 1:num_plot_cells
        ...
        cur_cluster = cluster_matching_perm(cur_cell);

        if cur_cluster > true_num_clusters
            init_waveform = zeros(size(CBPdata.CBP.init_waveforms{1}));
        else
            init_waveform = CBPdata.CBP.init_waveforms{cur_cluster}(:);
        end
        final_waveform = ...
            reshape(CBPdata.waveformrefinement.final_waveforms{cur_cell}, [], nchan);
    ...
    for n = 1:num_plot_cells
        ...
        subplot(nr, nc, n);
        cla;
        if cur_cluster > true_num_clusters  %if this is an "unassigned" cluster
            init_waveform = zeros(waveform_len, nchan);
        else
            init_waveform = ...
                reshape(CBPdata.CBP.init_waveforms{cur_cluster}, [], nchan);
        end
        final_waveform = ...
            reshape(CBPdata.waveformrefinement.final_waveforms{cur_cell}, [], nchan);
        ...
        err = norm(init_waveform - final_waveform, 'fro') / ...
                norm(CBPdata.waveformrefinement.final_waveforms{cur_cell}, 'fro');
        if cur_cluster > true_num_clusters  %if this is an "unassigned" cluster
            orig_cluster_id = "N/A";
        else
            orig_cluster_id = num2str(cur_cluster);
        end
        multiplottitle(sprintf('Cell %d (Orig #%s), Chg=%.0f%%, %d spikes', ...
                               cur_cell, orig_cluster_id, 100*err, ...
                               length(CBPdata.waveformrefinement.spike_time_array_thresholded{cur_cell})));
...
function splitCallback(varargin)
    ...
    dlgtext = inputdlg(['Enter waveform ID to split (1-' num2str(CBPdata.waveformrefinement.num_waveforms) '):'], ...
                        'Split CBP Waveforms');
...
function mergeCallback(varargin)
    global params;
    dlgtext = inputdlg(['Enter list of waveform IDs to merge (1-' num2str(params.clustering.num_waveforms) '), ' ...
                        'separated by space:'], ...
                        'Merge CBP Waveforms');
