...
% stack trace stuff
setparamifndef('params.plotting.raw_errors', 'false');
...
% threshold, rel. to max amplitude of 1, used to detect and remove spikes,
% estimating covariance of the remaining noise
setparamifndef('params.whitening.noise_threshold','0.15');
...
setparamifndef('params.clustering.spike_threshold','6');
% Window length - if empty => use general.spike_waveform_len
setparamifndef('params.clustering.window_len','[]');
% Peak length - if empty => use 0.5*window_len
setparamifndef('params.clustering.peak_len','[]');
...
% alignment mode for clustering only:
...
setparamifndef('params.clustering.averaging_mode','''maxabs''');
...
setparamifndef('params.cbp.magnitude_threshold','1e-2');
...
%%@ MIKE note - delete the "always FALSE" params above?
