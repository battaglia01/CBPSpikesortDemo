...
function under_panel = PyramidZoomMultiPlot(plots)
    ...
    scrollbar = uicontrol(get(under_panel,'Parent'),...
                           'style','slider',...
                           'units','normalized',...
                           'position',new_bar_pos);
    set(scrollbar, 'callback', @(varargin)scrollHandler(under_panel,scrollbar));

    % now create zoom buttons
    zoom_out_pos = [old_plot_pos(1),                      new_bar_pos(2), ...
                   old_plot_pos(3)*0.05,                 new_bar_pos(4)];
    zoom_in_pos  = [old_plot_pos(1)+old_plot_pos(3)*0.05, new_bar_pos(2), ...
                   old_plot_pos(3)*0.05,                 new_bar_pos(4)];
    sync_pos    = [old_plot_pos(1)+old_plot_pos(3)*0.90, new_bar_pos(2), ...
                   old_plot_pos(3)*0.1,                  new_bar_pos(4)];

    zoom_out = uicontrol(get(under_panel,'Parent'),...
                        'Style','pushbutton',...
                        'Units','normalized',...
                        'FontUnits','normalized',...
                        'String','-',...
                        'Position', zoom_out_pos,...
                        'Callback', @(varargin)zoomHandler(under_panel,-1));
    zoom_in  = uicontrol(get(under_panel,'Parent'),...
                        'Style','pushbutton',...
                        'Units','normalized',...
                        'FontUnits','normalized',...
                        'String','+',...
                        'Position', zoom_in_pos,...
                        'Callback', @(varargin)zoomHandler(under_panel,1));
    sync_all = uicontrol(get(under_panel,'Parent'),...
                        'Style','pushbutton',...
                        'Units','normalized',...
                        'FontUnits','normalized',...
                        'FontSize',0.6,...
                        'String','Sync',...
                        'Position', sync_pos);
    set(sync_all, 'Callback', @(varargin)syncHandler(under_panel, sync_all));
    ...
    rescaleScrollbar(scrollbar, initzoom, 0);

    setappdata(under_panel, 'zoomlevel', initzoom);
    setappdata(under_panel, 'xpos', 0);
    setappdata(under_panel, 'rescale', @(z,x) rescaleAll(under_panel, scrollbar, z, x));
...
%%@ ^^ NOTE: Metal no longer works on Mac R2019, so not necessary
    ...
    % Activate the under_panel axes (necessary for subplot and etc)
    axes(under_panel);
    
    % And now lastly, at the end, make sure we stack the scrollbar/UI
    % controls *above* the axes.
    %%@ NOTE: it is necessary to do this after calling drawnow; for some
    %%@ reason; probably an internal MATLAB bug
    uistack([scrollbar, zoom_in, zoom_out, sync_all], "up");
end
