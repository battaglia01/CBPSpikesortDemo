function [bandwidth,density,xmesh,cdf]= ...
    KernelDensityEstimator(data,n,MIN,MAX)
%
% Note: THIS WAS DOWNLOADED FORM MATLAB FORUM AND RENAMED. See:
...
%%@ Mike's addition - sometimes this is 0, causes crash
if sum(initial_data) ~= 0
    initial_data=initial_data/sum(initial_data);
end

%%@ Mike's addition - also, if we have only a single spike,
%%@ this can sometimes be a row vector when should be a col vector
if size(initial_data,2) > size(initial_data,1)
