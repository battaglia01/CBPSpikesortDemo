%%@ I'm not sure this function is needed anymore since we tend to do
%%@ thresholding elsewhere. Left for reference

function [total_misses, total_false_positives, total_true_positives, ...
          pruned_est_times, ...
          misses, false_positives, true_positives] = ...
    EvaluateSorting(est_times, est_amps, true_times, assignment_mtx, varargin)
...
pruned_est_times = PruneSpikes(est_times, est_amps, opts.threshold);

[total_misses, total_false_positives, total_true_positives, ...
 misses, false_positives, true_positives] = ...
    EvaluateSortingLowLevel(pruned_est_times, true_times, ...
                            assignment_mtx, opts.location_slack);
